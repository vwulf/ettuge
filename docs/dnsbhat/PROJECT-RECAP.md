...
| Book 15 dictionary entry boundaries | 15 | `book/kn/full.md` does not yet exist for book 15 — only `sample.md` (5,114L) and `raw.md` (284,713L). The raw WX-decoded OCR has no blank lines between entries. Fix: identify entry boundaries (English headword lines) and insert blank lines before each; produce `book/kn/full.md`. |
