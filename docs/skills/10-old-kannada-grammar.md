...
- **Rational** (ಉತ್ತಮ ಪ್ರಾಣಿ): people and deities → agreement suffix `-ru` (masculine
  honorific), `-ḷu` (feminine), `-ru` (plural)
- **Non-rational** (ಅನುತ್ತಮ / ಜಡ): animals, objects, abstracts → suffix `-itu` (sg),
  `-avu` (pl)
...
- Old Kannada's actual tense/aspect system is different: the `-guṃ/-kuṃ` suffix,
...
- Native Old Kannada clause-combining uses converbs (`-e` suffix, `-al` suffix)
