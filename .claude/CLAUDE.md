...
| `ettuge-content-qa` | "what books", "recommend a book on", "your notes on", "what did you save about", "your reflection on", "FP notes", "Haskell question", "what's in your knowledge base about" | Answers questions from Books (89+ influential files), FP (`pl/FP.md`, `haskell/`), and Reflection (20 topic files) |
| `ettuge-pre-deploy-check` | "sanity check", "pre-deploy check", "test before deploying", "check before pushing", "run the deploy check" | Runs pre-deployment sanity checks: correct `_config.yml`, `search_enabled:false`, REFL_CATS source files, no duplicate nav_order, chapter anchors, Python syntax |
