...
- **Sidebar structure (Phase 27):** `ಕನ್ನಡ` (root, nav_order=2) → `DNS Bhat` (L2) → one entry per book (L3, grand_parent=ಕನ್ನಡ); `Eke` (L2 sibling) → `Reference` (L3). All content files nav_excluded; navigation from book landing pages.
